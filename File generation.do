...
if "$country" == "I" global filename "Consistency Checks Batch $batch.xlsx";
if "$country" == "J" global filename "$area Consistency Checks Batch $batch.xlsx";

if "$country" == "I" global corrections "Consistency Checks Batch 5.xlsx";
if "$country" == "J" global corrections "All Consistency Checks Batch 1.xlsx";

global keep_vars Serial MEMID RR CAMP_YN;

if "$user" == "thomas" & "$country" == "J" global box "/Users/Thomas/Box Sync/Jordan SRHCS/WB Jordan IPSOS Data Exchange/";
if "$user" == "thomas" & "$country" == "I" global box "/Users/Thomas/Box Sync/KRI SRHCS/Kurdistan IPSOS Data Exchange";

if "$user" == "dhiraj" & "$country" == "J" global box "C:\Users\Wb425117\Box Sync\Jordan SRHCS\WB Jordan IPSOS Data Exchange";
if "$user" == "dhiraj" & "$country" == "I" global box "C:\Users\Wb425117\Box Sync\KRI SRHCS\Kurdistan IPSOS Data Exchange";

cd "$box/Main Survey Consistency Checks";
do programs;

/* BREAK : Are you sure you want to overwrite earlier files that took 30 minutes?!? */;
...
set obs 1;
gen issue = "";
saveold "$filename", replace v(13);

*********************;
*****  Import *******;
*********************;

if "$country" == "I" import excel using "$box/Main Survey Batch $batch.xlsx", clear first;
if "$country" == "J" import excel using "$box/$area Main Survey Batch $batch.xlsx", clear first;
...
rename (A27*A A27*B A27*C A27*D A27*E A27*F A27*G)
	(A27A* A27B* A27C* A27D* A27E* A27F* A27G*);
...
reshape long A9X A10X A11X A12X A13X A14X A15X A16_1X A16_2X A16_3X A17X A18X A19X A20X A21X 
	A22X A23X A24X A25X A26_1X A26_2X A26_3X A27AX A27BX A27CX A27DX A27EX A27FX 
	A27GX A28AX A28BX A29X A30X A31X, i(Serial) j(MEMBID);
...
if "$user" == "thomas" saveold "all", replace v(13);
if "$user" == "dhiraj" saveold "all", replace v(13);

keep Serial - ROSTERIDE8 ;
saveold "roster", replace v(13);
...
if "$user" == "thomas" saveold "hh_level", replace v(13);
if "$user" == "dhiraj" saveold "hh_level", replace v(13);
...
if "$user" == "thomas" saveold "main_respondent", replace v(13);
if "$user" == "dhiraj" saveold "main_respondent", replace v(13);

use "all", clear;
keep if K1 == MEMBID;
if "$user" == "thomas" saveold "selected_child", replace v(13);
if "$user" == "dhiraj" saveold "selected_child", replace v(13);
...
if "$user" == "thomas" saveold "RR2", replace v(13);
if "$user" == "dhiraj" saveold "RR2", replace v(13);
...
label var movedtoSyrian "Moved back to Syria";
...
if "$user" == "thomas" saveold "random_respondents", replace v(13);
if "$user" == "dhiraj" saveold "random_respondents", replace v(13);
